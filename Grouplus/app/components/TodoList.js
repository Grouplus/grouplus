...
class TodoList extends ParseComponent{
  constructor(props){
   ...
   // NetInfo.isConnected.fetch().done((connected) => {
    ...
    StorageHelper.get("todos"+this.props.group.objectId).then((value) =>{
           this.setState({"todoList": value.ongoingTodo, "todoDoneList": value.doneTodo});   
    });
  }
    ...
    var deleteBtn = {
      ...
      onPress: function(){
        ...
        var todosJSON = {doneTodo: that.data.todosDone, ongoingTodo: that.data.todos };
         StorageHelper.save("todos"+that.props.group.objectId, todosJSON);
